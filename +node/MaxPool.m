classdef MaxPool < handle
    ...
    methods
        ...
        function [] = ff(this)
            ...
            submat(:,:,:,:,2) = this.pI.O(2:2:end, 1:2:end, :, :);
            submat(:,:,:,:,3) = this.pI.O(1:2:end, 2:2:end, :, :);
        ...
        function [] = bp(this)
            ...
            % 求[H/2 W/2 C N 4] [H/2*W/2*C*N 4]下的坐标
            [H, W, C, N] = size(this.pI.O);
            submat = zeros([H/2 W/2 C N 4]);
            submat(:,:,:,:,1) = this.pI.O(1:2:end, 1:2:end, :, :);
            submat(:,:,:,:,2) = this.pI.O(2:2:end, 1:2:end, :, :);
            ...
            % this.gradO写入[H/2 W/2 C N 4]稀疏矩阵
            submat = zeros([H/2*W/2*C*N 4]);
            submat(idx) = this.gradO;
            submat = reshape(submat, [H/2 W/2 C N 4]);
            
            % [H/2 W/2 C N 4]稀疏矩阵转[H W C N]稀疏矩阵
            this.pI.gradO(1:2:end, 1:2:end, :, :) = submat(:,:,:,:,1);
            this.pI.gradO(2:2:end, 1:2:end, :, :) = submat(:,:,:,:,2);
            this.pI.gradO(1:2:end, 2:2:end, :, :) = submat(:,:,:,:,3);
